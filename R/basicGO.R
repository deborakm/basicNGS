#' basicGO: GO enrichment analysis using gprofiler2
#'
#' @description
#' Performs Gene Ontology pathway enrichment using gprofiler2::gost(),
...
#' @param sources Ontology source to query. Default is "GO".
#' @param pval_cutoff P-value cutoff (default = 0.05).
#' @param top Number of GO terms to keep (default = 50).
...
#'
#' @return A ggplot object representing the GO dotplot.
#' @export
#'
basicGO <- function(
    genes,
    ...
    top = 50,
    save_plot = FALSE,
    plot_path = "GO_dotplot.png",
    save_xlsx = FALSE,
    xlsx_path = "GO_results.xlsx"
) {
  ...
  if (is.null(gost_results)) {
    stop("Nenhum resultado encontrado para GO.")
  }
  ...
  # --- Build plot ----------------------------------------------------------------
  plot_GO <- ggplot2::ggplot(df, ggplot2::aes(
